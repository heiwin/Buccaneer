...
import { addTorrent, getTorrentDetails, streamWithVlc } from '../api/torrent';
...
export function TorrentActionMenu({ torrent, onClose, hideFileSelection }: TorrentActionMenuProps) {
  ...
  const handleStreamVlc = async () => {
    ...
    try {
      ...
      const fileIndex = onlyFiles && onlyFiles.length > 0 ? onlyFiles[0] : 0;
      const streamUrl = `http://127.0.0.1:3030/torrents/${id}/stream/${fileIndex}`;
      ...
      } catch { /* fallback — VLC will show the URL */ }
      await streamWithVlc(streamUrl, settings.vlcPath || null, title);
      onClose();
    } catch (e: unknown) {
      console.error(e instanceof Error ? e.message : String(e));
      await message('Failed to launch VLC', { title: 'Error', kind: 'error' });
    } finally {
      setLoading(false);
    }
  };
